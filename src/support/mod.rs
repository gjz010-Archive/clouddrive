...
#[async_trait]
pub trait CloudProvider: Sized+Send+Sync {
    ...
    async fn write(&mut self, offset: usize, buf: &[u8], write_through: bool)->std::io::Result<()>{
        if !self.bound_and_align_check(offset, buf.len()) {
...
pub trait CloudProviderExt{
    fn bound_and_align_check(&self, offset: usize, size: usize)->bool;
...
pub struct MutexProvider<T: CloudProvider>{
    provider: Mutex<T>,
    block_size: usize,
    total_size: usize
}
...
#[async_trait]
impl<T: CloudProvider + Send+Sync> SharedProvider for MutexProvider<T>{
    fn total_size(&self)->usize{
        self.total_size
    }
    async fn write(&self, offset: usize, buf: &[u8], write_through: bool)->std::io::Result<()>{
